...
public class Texture : IDisposable
    {
        ...
        public unsafe Texture(GL silkGl, string path)
        {
            ...
            using (var img = Image.Load<Rgba32>(path))
            {
                ...
                _silkGl.TexImage2D(TextureTarget.Texture2D, 0, InternalFormat.Rgba8, (uint) img.Width, (uint) img.Height, 0, PixelFormat.Rgba, PixelType.UnsignedByte, null);
                ...
                img.ProcessPixelRows(accessor =>
                {
                    ...
                    for (int y = 0; y < accessor.Height; y++)
                    {
                        fixed (void* data = accessor.GetRowSpan(y))
                        {
                            ...
                            _silkGl.TexSubImage2D(TextureTarget.Texture2D, 0, 0, y, (uint) accessor.Width, 1, PixelFormat.Rgba, PixelType.UnsignedByte, data);
                        }
                    }
                });
            }
        ...
        public unsafe Texture(GL silkGl, Span<byte> data, uint width, uint height)
        {
            ...
            fixed (void* d = &data[0])
            {
                ...
                _silkGl.TexImage2D(TextureTarget.Texture2D, 0, (int) InternalFormat.Rgba, width, height, 0, PixelFormat.Rgba, PixelType.UnsignedByte, d);
                SetParameters();
            }
        }
